...
const schema = z.object({
  principal: z.number().min(0).max(10_00_00_00_000),
  annualContribution: z.number().min(0).max(10_00_00_00_000),
...
const defaults: FormValues = {
  principal: 25_00_000,
  annualContribution: 5_00_000,
  years: 20,
  rate: 11,
};

export function FutureValueCalculator() {
  const meta = getCalculatorBySlug("future-value")!;
  ...
  const result = useMemo(() => {
    ...
    return calcFutureValue(parsed.data);
  }, [values]);
    ...
    <CalculatorShell
      ...
      form={
        <div className="flex flex-col gap-6">
          ...
          <NumberField
            ...
            hint="Set to zero for a pure starting-corpus projection"
            error={formState.errors.annualContribution?.message}
          />
          <NumberField
            label="Duration"
            unit="Years"
      ...
      result={
        <ResultPanel
          ...
          footnote="Useful for estate-planning conversations — projects a starting corpus with steady top-ups forward in time."
        />
      }
    />
  );
